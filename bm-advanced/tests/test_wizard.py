...
class TestWizard(TransactionCase):
    def setUp(self, *args, **kwargs):
        ...
        # 关闭开放状态的bug
        self.env["bm.bug"].search([("is_closed", "=", False)]).write(
            {"is_closed": True}
        )
        # 选择用demo用户的身份来运行测试
        demo_user = self.env.ref("base.user_demo")
        Todo = self.env["bm.bug"].with_user(demo_user)
        # 创建两个bug用于测试
        t0 = date.today()
        ...
        # 创建向导用例
        Wizard = self.env["bug.wizard"].with_user(demo_user)
        self.wizard = Wizard.with_context(active_ids=None).create({})
    ...
    def test_populate_tasks(self):
        ...
        self.assertEqual(count, 2, "Expected 2 populated bugs")

    def test_do_close(self):
        """批量关闭bug"""
        self.wizard.do_populate_tasks()
        self.wizard.do_close()
        self.assertEqual(self.bug1.is_closed, self.bug2.is_closed)
